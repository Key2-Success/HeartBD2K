...
# load data
load(file = "Kitu/College/Junior Year/Extracurriculars/Data Science Research Internship/sig_RN_parsed_z-test_all_RN.Rdata")
...
save (mtx_manual_choose, file = "Kitu/College/Junior Year/Extracurriculars/Data Science Research Internship/mtx_sig_RN_ztest_allRN.Rdata")
load(file = "Kitu/College/Junior Year/Extracurriculars/Data Science Research Internship/mtx_sig_RN_ztest_allRN.Rdata")
...
save(diffs, file = "Kitu/College/Junior Year/Extracurriculars/Data Science Research Internship/diffs_all_sig_RN.Rdata")
...
top10diffs <- diffs[1:17, ] # changed from diffs[diffs$word %in% df_pval2_sig$word[1:30], ] OR diffs[1:30]
top10diffs <- top10diffs[-c(9, 16), ] # remove "human" and "cholesterol"
top10diffs <- top10diffs[1:15, ]
...
ggplot(top10diffs, aes(x = " ", y = words, fill = scaled)) + geom_tile() + 
  xlab(" ") + ylab("RN terms") + labs(title = "Correlation of Most Distinct RN terms by Gender") + 
...
# save as an svg file
ggsave(file = "Kitu/College/Junior Year/Extracurriculars/Data Science Research Internship/15_most_distinct_RN.svg")
